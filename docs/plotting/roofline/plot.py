...
def addPerfLine(peakPerf, label):
    ...
    ax.text(0.76, yCoordinateTransformed + 0.01, label + " (" + str(peakPerf) + " F/C)", fontsize=8,
...
def addBWLine(BW, label):
    ...
    ax.text(0.01, yCoordinateTransformed + 0.008 * (len(str(BW)) - 1), label + ' (' + str(BW) + ' B/C)', fontsize=8,
...
with open('./roofline_data.csv', 'r') as input_data:
...
for serie in data.keys():
    ...
    ax.annotate(serie,
                xy=(x[0], y[0]), xycoords='data',
                xytext=(-12, -9), textcoords='offset points', fontsize=8)
