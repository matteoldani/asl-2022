            ...
            for (k = 0; k < A_n_col - nB + 1; k+=nB) {
                ...
                for (int ii = i; ii < i + nB; ii++) {
                    for (int jj = j; jj < j + nB; jj++) {
                        ...
                        for (kk = k; kk < k + nB - unrolling_factor + 1; kk+= unrolling_factor){
                            //printf("ii:%d, jj:%d k:%d kk: %d\n",ii, jj, k, kk);
                            R_Rij0 += A[Aii + kk  ] * B[kk       * B_n_col + jj];
                            R_Rij1 += A[Aii + kk+1] * B[(kk + 1) * B_n_col + jj];
                            R_Rij2 += A[Aii + kk+2] * B[(kk + 2) * B_n_col + jj];
                            R_Rij3 += A[Aii + kk+3] * B[(kk + 3) * B_n_col + jj];
                            R_Rij4 += A[Aii + kk+4] * B[(kk + 4) * B_n_col + jj];
                            R_Rij5 += A[Aii + kk+5] * B[(kk + 5) * B_n_col + jj];
                            R_Rij6 += A[Aii + kk+6] * B[(kk + 6) * B_n_col + jj];
                            R_Rij7 += A[Aii + kk+7] * B[(kk + 7) * B_n_col + jj];
                        }
                        R[Rij] += R_Rij0 + R_Rij1 + R_Rij2 + R_Rij3 + R_Rij4 + R_Rij5 + R_Rij6 + R_Rij7; 
                        for (; kk < k + nB; kk++){
