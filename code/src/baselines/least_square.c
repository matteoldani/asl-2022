...
void
nlssubprob(double *V, double *W, double *Hinit, int m, int n, int k, double tol, int maxiter, double *H, double *grad,
           int *ite) {
...
void
NMF(double *V, double *Winit, double *Hinit, int m, int n, int k, double tol, double timelimit, int maxiter, double *W,
    double *H) {
    ...
    for (iter = 1; iter <= maxiter; iter++) {
        ...
        transpose(H, Ht, n, k);
        transpose(W, Wt, m, n);
        nlssubprob(Vt, Ht, Wt, k, n, m, tolW, 10, W, gradW, &iterW);//printf("complete stage %d\n",iter);
        transpose(W, W, m, n);
        ...
        int iterH = 0;
        nlssubprob(V, W, H, m, n, k, tolH, 10, H, gradH, &iterH);//printf("complete stage %d\n",iter);
        if (iterH == 1)
            tolH = 0.1 * tolH;
...
int main(int argc, char **argv) {
    ...
    if (!(argc == 4 || (argc == 2 && atoi(argv[1]) == -1))) {
        printf("least_square <m> <n> <r>\n");
        printf("least_square -1 (measure runtime for incresing size matrices)\n");
        return -1;
    }
    if (argc == 2) {
        int input_sizes[NUM_SAMPLES] = {100, 250, 500, 750, 1000, 1250, 1500};
        ...
        for (int i = 0; i < NUM_SAMPLES; i++) {
            ...
            clock_t start, end;
            start = clock();
            NMF(V, Winit, Hinit, m, n, k, -1, 9999999999999999, 1000, W, H);
            end = clock();
    ...
    NMF(V, Winit, Hinit, m, n, k, -1, 100000000, 1000, W, H);
    end = clock();
    printf("m: %d n:%d r:%d, time:%.6lf s\n", m, k, n, ((end - start) / (double) CLOCKS_PER_SEC));
    // printf("%i\n", m);
    // printf("%f\n", ((end - start) / (double) CLOCKS_PER_SEC));
    ...
    //OutPutVector(m*n, W);
    //printf("\n");
    //printf("values in H (column major):\n");
    //OutPutVector(n*k, H);
    //printf("\n");

    double *Vlast = 0;
    Vlast = (double *) malloc(m * k * sizeof(double));
    memset(Vlast, 0, m * k * sizeof(double));
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, m, k, n, 1, W, m, H, n, 0, Vlast, m);
