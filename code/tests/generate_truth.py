...
def generate_mul(m, n, r):
    ...
    R = np.matmul(A, B)
    np.savetxt(input_files + f"/A_mul_{m}_{n}_{r}.matrix", A, fmt="%.12f", header=f"{A.shape[0]} {A.shape[1]}",
               comments="")
    np.savetxt(input_files + f"/B_mul_{m}_{n}_{r}.matrix", B, fmt="%.12f", header=f"{B.shape[0]} {B.shape[1]}",
               comments="")
    np.savetxt(input_files + f"/R_mul_{m}_{n}_{r}.matrix", R, fmt="%.12f", header=f"{R.shape[0]} {R.shape[1]}",
               comments="")
